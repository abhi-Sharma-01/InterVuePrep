        ...
        const unsubscribeSnapshot = onSnapshot(
          ...
          (snapshot) => {
            const interviewsData = snapshot.docs
              .map((doc) => {
                const data = doc.data();
                const timestampValue = data.timestamp?.toDate?.() || null;
                return {
                id: doc.id,
                ...
                date:
                  data.date ||
                  (timestampValue ? timestampValue.toLocaleDateString() : ""),
                status:
                  data.status ||
                  (data.feedback || data.overallScore != null
                    ? "Completed"
                    : "Pending"),
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full relative">
            ...
            {selectedInterview.strengths && (
              <p className="text-sm mt-2 text-gray-700">
                <strong>Strengths:</strong> {selectedInterview.strengths}
              </p>
            )}
