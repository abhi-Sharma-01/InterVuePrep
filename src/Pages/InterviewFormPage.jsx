...
Only return valid JSON.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const rawText = (await generateQuestions(prompt)).trim();
      ...
      let message = "Failed to generate questions or save data. Please try again.";

      if (isRetriableGeminiError(error)) {
        message =
          "Gemini is under heavy load right now. The app retried automatically, but the request still did not go through. Please wait a moment and try again.";
      } else if (
        error?.message?.includes("not found") ||
        error?.message?.includes("supported for generateContent")
      ) {
        message =
          "Interview question generation failed because the AI model is unavailable. Please try again.";
      }
